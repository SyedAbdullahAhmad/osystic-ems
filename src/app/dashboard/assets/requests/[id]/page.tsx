"use client";
// src/app/dashboard/assets/requests/[id]/page.tsx
//
// Mirrors src/app/dashboard/attendance/corrections/[id]/page.tsx,
// including reusing workflow.service.ts as-is for Approve/Reject (see
// that file's header - built deliberately module-agnostic). Eligibility
// to approve/reject is enforced server-side by
...
//
// The one thing genuinely new here vs. Leave/Attendance:
// <FulfillAssetPanel> renders below the approval history once the
// request reaches APPROVED, letting an ASSET_MANAGE holder hand over a
// specific physical unit - a deliberate separate step from approval
// itself (see assets.fulfill_asset_request()'s header).
...
import { FulfillAssetPanel } from "@/components/assets/FulfillAssetPanel";
import { useAssetRequest, useAssetRequestApprovalHistory } from "@/hooks/useAssetRequests";
...
export default function AssetRequestDetailPage({ params }: PageProps) {
  const { id } = use(params);
  const { data: request, isLoading, refetch } = useAssetRequest(id);
  const { data: history } = useAssetRequestApprovalHistory(id);
  ...
  if (!request) return <p className="text-sm text-gray-500">Asset request not found.</p>;
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-2 text-sm text-gray-700 dark:text-gray-300">{request.justification}</p>
        <p className="mt-1 text-xs text-gray-400">
          Submitted {request.submittedAt ? format(parseISO(request.submittedAt), "MMM d, yyyy HH:mm") : "—"}
        </p>
      </div>
